...
type data = { buffer: bytes; length:int; position:int }
...
let create n =
 ...
 { data = { buffer = s; length = n; position = 0}; initial_buffer = s}

let contents b = Bytes.sub_string b.data.buffer 0 b.data.position
let to_bytes b = Bytes.sub b.data.buffer 0 b.data.position

let sub b ofs len =
  if ofs < 0 || len < 0 || ofs > b.data.position - len
  then invalid_arg "Buffer.sub"
  else Bytes.sub_string b.data.buffer ofs len


let blit src srcoff dst dstoff len =
  let {buffer; position; _ } = src.data in
  if len < 0 || srcoff < 0 || srcoff > position - len
             || dstoff < 0 || dstoff > (Bytes.length dst) - len
  then invalid_arg "Buffer.blit"
  else
    Bytes.unsafe_blit buffer srcoff dst dstoff len


let nth b ofs =
  let {position; buffer; _ } = b.data in
  if ofs < 0 || ofs >= position then
   invalid_arg "Buffer.nth"
  else Bytes.unsafe_get buffer ofs


let length b = b.data.position

let clear b =
  let data = b.data in
  b.data <- { data with position = 0 }

let reset b =
  let data = {
    buffer = b.initial_buffer;
    length = Bytes.length b.initial_buffer;
    position = 0;
  }
  in
  b.data <- data
...
let resize b more =
  let data = b.data in
  let old_pos = data.position in
  let old_len = data.length in
  ...
  Bytes.blit data.buffer 0 new_buffer 0 data.position;
  let data = { buffer = new_buffer; length = !new_len; position = data.position } in
  b.data <- data;
  assert (data.position + more <= data.length);
  assert (old_pos + more <= data.length);
  ()
...
let add_char b c =
  let {buffer; length; position=pos} = b.data in
  if pos >= length then begin
    resize b 1;
    Bytes.set b.data.buffer pos c
  end else
    Bytes.unsafe_set buffer pos c;
  let data = { b.data with position = pos + 1 } in
  b.data <- data
...
let rec add_utf_8_uchar b u =
  let data = b.data in
  let pos = data.position in
  if pos >=  data.length then resize b uchar_utf_8_byte_length_max;
  let n = Bytes.set_utf_8_uchar b.data.buffer pos u in
  if n = 0
  then (resize b uchar_utf_8_byte_length_max; add_utf_8_uchar b u)
  else (
    let data = { b.data with position = pos + n } in
    b.data <- data
  )

let rec add_utf_16be_uchar b u =
  let pos = b.data.position in
  if pos >= b.data.length then resize b uchar_utf_16_byte_length_max;
  let n = Bytes.set_utf_16be_uchar b.data.buffer pos u in
  if n = 0
  then (resize b uchar_utf_16_byte_length_max; add_utf_16be_uchar b u)
  else (b.data <- { b.data with position = pos + n})

let rec add_utf_16le_uchar b u =
  let pos = b.data.position in
  if pos >= b.data.length then resize b uchar_utf_16_byte_length_max;
  let n = Bytes.set_utf_16le_uchar b.data.buffer pos u in
  if n = 0
...
let add_substring b s offset len =
  ...
  let {position;buffer;length} = b.data in
  let new_position = position + len in
  if new_position > length then begin
    resize b len;
    Bytes.blit_string s offset b.data.buffer b.data.position len;
  end else
    Bytes.unsafe_blit_string s offset buffer position len;
  b.data <- { b.data with position = new_position }
...
let add_string b s =
  ...
  let {buffer; length; position} = b.data in
  let new_position = position + len in
  if new_position > length then begin
    resize b len;
    Bytes.blit_string s 0 b.data.buffer b.data.position len
  end else
...
let add_buffer b bs =
  add_subbytes b bs.data.buffer 0 bs.data.position
...
let unsafe_add_channel_up_to b ic len =
  if b.data.position + len > b.data.length then resize b len;
  let n = really_input_up_to ic b.data.buffer b.data.position len in
  ...
  assert (b.data.position + n <= b.data.length);
  b.data <- { b.data with position = b.data.position + n };
  n
...
let output_buffer oc b =
  output oc b.data.buffer 0 b.data.position
...
let truncate b len =
    if len < 0 || len > length b then
      ...
      b.data <- { b.data with position = len }
...
let to_seq b =
  let rec aux i () =
    ...
    if i >= b.data.position then Seq.Nil
    else
      let x = Bytes.get b.data.buffer i in
      Seq.Cons (x, aux (i+1))
  in
  aux 0
...
let add_int8 b x =
  let new_position = b.data.position + 1 in
  if new_position > b.data.length then resize b 1;
  unsafe_set_int8 b.data.buffer b.data.position x;
  b.data <- { b.data with position = new_position }

let add_int16_ne b x =
  let new_position = b.data.position + 2 in
  if new_position > b.data.length then resize b 2;
  unsafe_set_int16 b.data.buffer b.data.position x;
  b.data <- { b.data with position = new_position }

let add_int32_ne b x =
  let new_position = b.data.position + 4 in
  if new_position > b.data.length then resize b 4;
  unsafe_set_int32 b.data.buffer b.data.position x;
  b.data <- { b.data with position = new_position }

let add_int64_ne b x =
  let new_position = b.data.position + 8 in
  if new_position > b.data.length then resize b 8;
  unsafe_set_int64 b.data.buffer b.data.position x;
  b.data <- { b.data with position = new_position }
