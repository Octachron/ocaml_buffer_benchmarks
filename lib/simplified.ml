...
let create n =
 ...
 {buffer = s; position = 0; initial_buffer = s}
...
let resize b more =
  ...
  b.buffer <- new_buffer;
  assert (b.position + more <= Bytes.length b.buffer);
  assert (old_pos + more <= Bytes.length b.buffer);
  ()
...
let rec add_utf_8_uchar b u =
  let pos = b.position in
  if pos >= Bytes.length b.buffer then resize b uchar_utf_8_byte_length_max;
...
let rec add_utf_16be_uchar b u =
  let pos = b.position in
  if pos >= Bytes.length b.buffer then resize b uchar_utf_16_byte_length_max;
...
let unsafe_add_channel_up_to b ic len =
  if b.position + len > Bytes.length b.buffer then resize b len;
  ...
  assert (b.position + n <= Bytes.length b.buffer);
