...
(* [extended_inner_buffer b more] ensures that [b.position + more <= b.inner.length] holds
   by dynamically extending [b.inner] if necessary -- and thus
   increasing [b.inner.length].
   The function returns the extended inner buffer.
*)
let extended_inner_buffer b more =
  ...
  let new_inner = { buffer = new_buffer; length = !new_len } in
  b.inner <- new_inner;
  assert (b.position + more <= b.inner.length);
  assert (old_pos + more <= b.inner.length);
  new_inner

let resize b more = ignore (extended_inner_buffer b more)
...
let add_char b c =
  ...
  let inner = b.inner in
  let {buffer; _} =
    if pos >= inner.length then extended_inner_buffer b 1 else inner in
...
let add_substring b s offset len =
  ...
  let {buffer; _} = if new_position > inner.length then extended_inner_buffer b len else inner
  in
...
let add_string b s =
  ...
  let {buffer; _} =
    if new_position > inner.length then extended_inner_buffer b len else inner
  in
