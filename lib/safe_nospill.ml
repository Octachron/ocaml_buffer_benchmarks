(* Changes: use only safe function in add_char and add_string *)
...
let add_substring b s offset len =
  ...
  else  Bytes.blit_string s offset b.buffer b.position len;
...
let add_string b s =
  ...
  else Bytes.blit_string s 0 b.buffer b.position len;
  b.position <- new_position
