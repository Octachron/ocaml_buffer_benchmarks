...
let contents b = Bytes.sub_string b.data.buffer 0 b.position
let to_bytes b = Bytes.sub b.data.buffer 0 b.position
...
let blit src srcoff dst dstoff len =
  ...
  then invalid_arg "Buffer.blit"
    ...
    Bytes.unsafe_blit src.data.buffer srcoff dst dstoff len


let nth b ofs =
  if ofs < 0 || ofs >= b.position then
   invalid_arg "Buffer.nth"
  else Bytes.unsafe_get b.data.buffer ofs
...
let resize b more =
  ...
  let data = { buffer = new_buffer; length = !new_len } in
  b.data <- data;
...
let rec add_utf_8_uchar b u =
  let pos = b.position in
  if pos >= b.data.length then resize b uchar_utf_8_byte_length_max;
...
let add_substring b s offset len =
  ...
  if new_position > length then begin
    resize b len;
    Bytes.blit_string s offset b.data.buffer b.position len;
  end else
    Bytes.unsafe_blit_string s offset buffer b.position len;
...
let add_string b s =
  ...
  if new_position > length then begin
    resize b len;
    Bytes.blit_string s 0 b.data.buffer b.position len
  end else
    Bytes.unsafe_blit_string s 0 buffer b.position len;
  b.position <- new_position

let add_bytes b s = add_string b (Bytes.unsafe_to_string s)

let add_buffer b bs =
  add_subbytes b bs.data.buffer 0 bs.position
...
let unsafe_add_channel_up_to b ic len =
  if b.position + len > b.data.length then resize b len;
  let n = really_input_up_to ic b.data.buffer b.position len in
  ...
  assert (b.position + n <= b.data.length);
...
let output_buffer oc b =
  output oc b.data.buffer 0 b.position
